...
from models.modules.misc import ReLU, InSequence, Real, make_sws_transformer, \
    make_2d_equiangular_grid_in_3d_euclidian_space
from models.modules.residual_blocks import SWSPreActivationResidualBlock3D, ResConvBlock
from models.modules.set_convolution import TruncateContextSetDecorator, BatchedSetConvolution, \
    ExponentialKernelSetConvolution, SphericalGaussianKernel, SwapIpsiContraDecorator1, SwapIpsiContraDecorator2
...
def sconvcnp_factory(use_running_stats, resolution, input_set_convolution_kwargs, max_context_datapoint_count,
    ...
    def cnn_module():
        ...
        dcm = partial(InSequence.constructor,
                      [partial(nn.Conv,
                               features=features,
                               kernel_size=(1, 1, 1, 1),
                               # shape is (batch, resolution, resolution, sequence, spin, channel)
                               name='resizer')] + \
    ...
    def pointwise_decoder_module():
        ...
        if resizer_decoder_layer:
            ...
            ddm = partial(InSequence.constructor,
                          [partial(nn.Conv,
                                   features=features,
                                   kernel_size=(1, 1, 1),
                                   # shape is (batch, datapoints, sequence, spin, channel)
                                   padding='SAME',
                                   use_bias=True,
                                   name='resizer')] + \
                          hidden_blocks + \
    ...
    def input_set_convolution_module():
        ...
        scm = partial(ExponentialKernelSetConvolution,
        ...
        module = partial(TruncateContextSetDecorator,
                         ...
                         set_convolution_module=partial(BatchedSetConvolution,
                                                        set_convolution_module=scm,
                                                        **{key: value for key, value in
                                                           input_set_convolution_kwargs.items()
    ...
    def latent_set_convolution_module():
        ...
        scm = partial(ExponentialKernelSetConvolution,
                      ...
                      initializer=partial(constant_value_initialiser,
                                          # TODO: make constant a user definable argument
                                          constant=softplus_inverse(2 / resolution / 10)),
                      return_density=latent_set_convolution_kwargs['return_density'],
                      share_scale_across_data_and_density=latent_set_convolution_kwargs['share_scale_across_data_and_density'])

        module = partial(BatchedSetConvolution,
                         set_convolution_module=scm,
                         **{key: value for key, value in
                            latent_set_convolution_kwargs.items()
                            if key not in ['return_density', 'share_scale_across_data_and_density']})
    ...
    return ModelDecorator(input_preprocessing_mode=input_preprocessing,
                          ...
                          model_module=partial(ConvCNP,
                                               grid=make_2d_equiangular_grid_in_3d_euclidian_space(
                                               ...
                                               sigma_transform=lambda sigma: sigma_floor + (
                                                       1 - sigma_floor) * nn.softplus(sigma),
